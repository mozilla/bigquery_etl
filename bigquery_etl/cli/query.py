...
from ..query_scheduling.task import Task
...
def create(name, sql_dir, project_id, owner, init, task_name):
    ...
    # validate that the (possibly autogenerated) task name is within
    # specification, if not we should tell the user to pick one within it
    if not task_name:
        task_name = Task.generate_task_name(dataset, name, version)
    try:
        Task.validate_task_name(task_name)
    except ValueError as e:
        click.echo(e)  # Gives the error
        click.echo("You can specify an alternate task name with --task-name")
        sys.exit(1)
    ...
    metadata = Metadata(
        ...
        scheduling=dict(task_name=task_name) if task_name else {},
    )
...
def schedule(name, sql_dir, project_id, dag, depends_on_past):
    ...
    for query_file in query_files:
        ...
        if dag:
            ...
            # write additional scheduling information to metadata file
